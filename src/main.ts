...
import { Perceptron } from "./neural network/neurons/perceptron";
import { LinearNeuron } from "./neural network/neurons/linear";
import { SigmoidNeuron } from "./neural network/neurons/sigmoid";
...
let myPerceptron = new Perceptron({
  inputs: [1, 0, 0],
  weights: [6, 3, 3],
  threshold: 4
});
...
// introducing bias
let myPerceptron2 = new Perceptron({
  inputs: [1, 0, 0],
  weights: [6, 3, 3],
  bias: -5
});
...
let myLinearNeuron = new LinearNeuron({
  inputs: [1, 2],
  weights: [5, 2],
  bias: 2
});
...
let mySigmoidNeuron = new SigmoidNeuron({
  inputs: [0.2, 0.2],
  weights: [1, 1],
  bias: 0
});
...
let learningPerceptron = new Perceptron({
  inputs: [1, 1],
  learningRate: 0.5
});
